...
func TestTestifyLikeTest(t *testing.T) {
	...
	t.Run("check_suite_registration", func(t *testing.T) {
		...
		if tests, ok := testifyTestsByParentT[reflect.ValueOf(tParent).UnsafePointer()]; ok {
			...
			if test.moduleName != "github.com/kakkoyun/dd-trace-go/v2/internal/civisibility/integrations/gotesting" {
				t.Errorf("Expected module name to be github.com/kakkoyun/dd-trace-go/v2/internal/civisibility/integrations/gotesting, got %s", test.moduleName)
			}
		} else {
