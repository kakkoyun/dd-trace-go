...
func tagAndPush(dryRun, disablePush bool, remote string, root, mod GoMod, version string) error {
	...
	existingTag, err := runCommandWithOutput(mod.dir, "git", "tag", "--list", tagName)
	if err != nil {
		return fmt.Errorf("failed to check tags for %s: %w", mod.Module.Path, err)
	}
	if strings.TrimSpace(existingTag) != "" {
		slog.Info("Tag already exists; skipping tag creation", "module", mod.Module.Path, "tag", tagName)
		return nil
	}

	// Create the tag
	err = runCommand(mod.dir, "git", "tag", "-am", tagName, tagName)
	if err != nil {
		return err
	}
	...
	return runCommand(mod.dir, "git", "push", remote, tagName)
}
...
func filterModules(modules map[string]GoMod, rootModulePath string, excludedModules []string) map[string]GoMod {
	...
	for path, mod := range modules {
		...
		if contains(excludedModules, path) {
			continue
		}
...
func findModules(root string, excludedDirs []string) (map[string]GoMod, error) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, err error) error {
		...
		if entry.IsDir() && (contains(excludedDirs, path) || entry.Name() == ".git") {
...
// contains checks if a string appears in a slice.
func contains(list []string, s string) bool {
	for _, item := range list {
		eql, err := normalizedEqual(item, s)
		if err != nil {
...
// normalizedEqual checks if two paths are equal after normalizing and converting to absolute paths.
func normalizedEqual(path1, path2 string) (bool, error) {
