...
func TestLogWriter(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		...
		s := makeSpan(0)
		for i := 0; i < 20; i++ {
			h.add(serializableTrace{s.snapshot(), s.snapshot()})
		}
		h.flush()
	...
	t.Run("inf+nan", func(t *testing.T) {
		...
		h := newLogTraceWriter(cfg, statsd)
		h.w = &buf
		var s recordingSpan = makeSpan(0)
		s.setMetric("nan", math.NaN())
		s.setMetric("+inf", math.Inf(1))
		s.setMetric("-inf", math.Inf(-1))
		h.add(serializableTrace{s.snapshot()})
		h.flush()
		json := buf.String()
...
func TestLogWriterOverflow(t *testing.T) {
	...
	t.Run("split", func(t *testing.T) {
		...
		h.w = &buf
		s := makeSpan(10)
		var trace serializableTrace
		for i := 0; i < 500; i++ {
			trace = append(trace, s.snapshot())
		}
		h.add(trace)
		h.flush()
		v := struct{ Traces [][]map[string]interface{} }{}
...
func TestTraceWriterFlushRetries(t *testing.T) {
	...
	ss := serializableTrace{makeSpan(0).snapshot()}
...
func BenchmarkJsonEncodeSpan(b *testing.B) {
	var s recordingSpan = makeSpan(10)
	s.setMetric("nan", math.NaN())
	s.setMetric("+inf", math.Inf(1))
	s.setMetric("-inf", math.Inf(-1))
	snapshot := s.snapshot()
